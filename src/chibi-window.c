...
G_DEFINE_TYPE (
  ChibiWindow, chibi_window, GTK_TYPE_APPLICATION_WINDOW)
...
static const char * carla_file_callback (
  void * const       ptr,
  ...
  ChibiApplication * app)
{
  ...
  char * proper_name =
    g_strdup_printf (
      "Chibi%s%s",
      g_ascii_isdigit (app->name[0]) ? "-" : "", app->name);
  ...
  carla_set_engine_option (
    self->host_handle, ENGINE_OPTION_OSC_ENABLED, 0,
    NULL);
  ...
  carla_set_engine_option (
    ...
    carla_set_file_callback (
      self->host_handle, carla_file_callback, self);

    // TODO set frontendWinId in case we cannot embed

    carla_set_engine_option (
      self->host_handle, ENGINE_OPTION_PREFER_UI_BRIDGES, 0, NULL);
    ...
    for (uint i = 0; i < driver_count; i++)
      {
        g_message (
          "found driver %u: %s", i, carla_get_engine_driver_name (i));
      }

    // NOTE: this assumes JACK driver for now
    bool ret =
      carla_engine_init (self->host_handle, "JACK", "Chibi");
    if (!ret)
      {
        g_error (
          "failed to init engine: %s",
...
static void
chibi_window_class_init (
  ChibiWindowClass * klass)
{
}

static void
chibi_window_init (ChibiWindow * self)
{
}
